...
export default function NewProductPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!image) {
      toast.error("Product image is required for creative assembly");
      return;
    }
    setLoading(true);
    try {
        ...
        <form onSubmit={submit} className="space-y-6 bg-white rounded-2xl p-8 shadow-sm ring-1 ring-slate-100">
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1">
              ...
              />
              <button type="button" onClick={addTag} className="rounded-lg border px-3 text-sm">
                Add
              </button>
            </div>
            <div className="flex flex-wrap gap-2 mt-2">
              {benefits.map((b) => (
                <span key={b} className="rounded-full bg-slate-100 px-3 py-1 text-xs">
                  {b}
                </span>
              ))}
            </div>
