        ...
        <form onSubmit={submit} className="space-y-6 bg-white rounded-2xl p-8 shadow-sm ring-1 ring-slate-100">
              ...
              {["professional", "playful", "bold", "luxury"].map((t) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm capitalize border ${
                    tone === t ? "border-indigo-600 bg-indigo-50 text-indigo-800" : "border-slate-200"
                  }`}
                >
                  {t}
                </button>
              ...
              {INTERESTS.map((i) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium border ${
                    picked.includes(i) ? "bg-indigo-600 text-white border-indigo-600" : "border-slate-200"
                  }`}
                >
                  {i}
                </button>
          ...
          <div>
            ...
            <div className="flex gap-4 mt-2">
              <input type="range" min={18} max={65} value={ageMin} onChange={(e) => setAgeMin(+e.target.value)} />
              <input type="range" min={18} max={65} value={ageMax} onChange={(e) => setAgeMax(+e.target.value)} />
            </div>
          </div>
          <div>
