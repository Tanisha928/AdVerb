  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const c = await apiPost<Campaign>(`/brands/${id}/campaigns`, {
        name, objective, budget: budget ? Number(budget) : null, start_date: null, end_date: null,
      });
        ...
        <form onSubmit={submit} className="space-y-6 bg-white rounded-2xl p-8 shadow-sm ring-1 ring-slate-100">
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {([["awareness", "Reach new audiences"], ["clicks", "Drive qualified traffic"], ["conversions", "Optimize for outcomes"]] as const).map(([k, d]) => (
                <button type="button" key={k} onClick={() => setObjective(k)}
                  className={`rounded-xl border p-4 text-left text-sm ${objective === k ? "border-indigo-600 bg-indigo-50" : "border-slate-200"}`}
                >
          ...
          <div>
            ...
            <input value={budget} onChange={(e) => setBudget(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" />
          </div>
