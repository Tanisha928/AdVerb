...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    try {
      setHistory(JSON.parse(localStorage.getItem(`adverb_clicks_${id}`) || "[]") as ClickRow[]);
    } catch { setHistory([]); }
  }, []);
  ...
  return (
    <div className="max-w-2xl mx-auto p-10">
      <Link href="/feed/browse" className="text-indigo-600 text-sm font-semibold">← Back to feed</Link>
      ...
      <div className="rounded-2xl border border-slate-200 p-6 bg-white mb-6">
        ...
        />
        <button type="button" onClick={saveInterests} className="mt-3 text-sm font-semibold text-indigo-600">Save interests</button>
      </div>
      <div className="rounded-2xl border border-slate-200 p-6 bg-white">
        <div className="flex justify-between items-center mb-4">
          <p className="text-sm font-semibold">Clicked ads</p>
          <button type="button" onClick={clearHistory} className="text-xs text-red-600 font-semibold">Clear history</button>
        </div>
