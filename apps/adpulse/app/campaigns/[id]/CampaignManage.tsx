...
export function CampaignManage({
  initialCampaign, initialProducts, initialCreatives,
}: {
  initialCampaign: Campaign; initialProducts: Product[]; initialCreatives: Creative[];
}) {
  ...
  const approvedByProduct = useMemo(() => {
    ...
    for (const c of creatives) if (c.status === "approved") m[c.product_id] = (m[c.product_id] || 0) + 1;
    return m;
  }, [creatives]);

  const canLaunch = useMemo(() => products.length > 0 && products.every((p) => (approvedByProduct[p.id] || 0) >= 2), [products, approvedByProduct]);
  ...
  async function refresh() {
    ...
    setCampaign(c); setProducts(p); setCreatives(cr);
  ...
  async function generate(productId?: string) {
    ...
    if (!targetProductId) { toast.error("Select a product first"); return; }
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          ...
          <div className="mt-4 flex gap-2">
            {steps.map((s, i) => (
              <div key={s} className="flex items-center gap-2 text-xs">
                <span className={`h-8 w-8 rounded-full flex items-center justify-center font-semibold ${i <= stepIndex ? "bg-indigo-600 text-white" : "bg-slate-200 text-slate-500"}`}>{i + 1}</span>
        ...
        <div className="flex gap-2">
          {products.length > 0 && (
            <select value={selectedProductId} onChange={(e) => setSelectedProductId(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
              {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          )}
          {products.length > 0 && (
            <button disabled={generatingProductId !== null || !selectedProductId} onClick={() => { void generate(); }}
              className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500 disabled:opacity-50"
            >
              {generatingProductId === selectedProductId ? "Generating…" : "Generate creatives"}
            </button>
          )}
          <Link href={`/campaigns/${campaign.id}/review`} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white">Review</Link>
          <button disabled={!canLaunch || launchBusy || generatingProductId !== null} onClick={launch}
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href={`/campaigns/${campaign.id}/products/new`} className="text-sm font-semibold text-indigo-600 hover:underline">Add product</Link>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {products.map((p) => (
          <div key={p.id} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              {(p.key_benefits || []).map((b) => <span key={b} className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px]">{b}</span>)}
            </div>
            <button type="button" disabled={generatingProductId !== null} onClick={() => generate(p.id)}
        ...
        {products.length === 0 && (
          <div className="col-span-full rounded-xl border border-dashed border-slate-200 p-8 text-center text-slate-500">Add products to enable creative generation.</div>
        )}
      </div>
    </div>
  );
